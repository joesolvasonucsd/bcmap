...
python collapseseq.py ${QFILTERED_BASENAME}.fastq # Script 1. Strips data down to the read and counts per read. This outputs collapsed.txt file

python combinecollapsefiles.py ${QFILTERED_BASENAME}.txt # Script 1.5.  Combines all the reads that are the same sequence. 

6.Python Split4parallele.py # Script 2 splits the data into smaller units so we can run in parallel
