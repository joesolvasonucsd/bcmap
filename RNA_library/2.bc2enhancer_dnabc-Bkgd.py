...
#				print DominantEnhancer
		...
		line_out+=temp
		if numUnique>0 and evaluator>0: # if (bc is unique) and (enhancer observed more than 0 times)
			line_out3+=temp 
...
dnaEn = df.loc[:,4+rnaCol].unique() # background is the unique set of all enhancers observed in DNAbc ($
...
open(dataDir+"barcode2enhancer2reads_uniqueAll4_dnabc_collapsed.txt","w").write(line_out)
...
line_out=""
ID=0
for line in open(dataDir+"barcode2enhancer2reads_uniqueAll4_dnabc_collapsed.txt").read().rstrip().split("\n"):
        a=line.split("\t")
